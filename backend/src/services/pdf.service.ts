import { IRemission, IItem } from '../models/Remission.js';
...
const styles: StyleDictionary = {
  ...
  trTitle: { fontSize: 13, bold: true, color: '#1f2937' },
  trText: { fontSize: 9, color: '#111827' },
  trSectionLabel: { fontSize: 8, bold: true, color: '#6b7280' },
  trSignature: { fontSize: 8, margin: [0, 5, 0, 0] },
  trTableHeader: { fontSize: 8, bold: true, color: '#ffffff', fillColor: '#1a56db', margin: [5, 2, 5, 2] },
};

const buildHeader = (remission: IRemission): Content => {
...
const buildInfoSection = (remission: IRemission): Content => {
  return {
    columns: [
      {
        width: '50%',
...
const buildItemsTable = (items: IItem[]): Content => {
  const tableHeader = [
...
const buildSignatures = (remission: IRemission): Content => {
  return {
    columns: [
      {
        stack: [
...
const buildResponsibilityTerm = (remission: IRemission): Content[] => {
  const trItems = remission.items.filter(item => item.requiresResponsibilityTerm);
  if (trItems.length === 0) return [];
...
export const generateRemissionPDF = (remission: IRemission): TDocumentDefinitions => {
  ...
  const definition: TDocumentDefinitions = {
    ...
    content: [
      buildHeader(remission),
      { canvas: [{ type: 'line', x1: 0, y1: 5, x2: 515, y2: 5, lineWidth: 1, lineColor: '#cccccc' }] },
      { text: '\n' },
      buildInfoSection(remission),
      { text: '\n' },
      buildItemsTable(remission.items),
      { text: '\n\n\n' },
      buildSignatures(remission),
      ...buildResponsibilityTerm(remission)
    ],
    styles,
    defaultStyle: {
      font: 'Roboto',
